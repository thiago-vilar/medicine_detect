...
def display_markers(image, corners, ids):
    ...
    for corner, id_ in zip(corners, ids.flatten()):
        ...
        cv2.putText(image, 'Scan Area', (x_min, y_min - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.4, hot_pink, 1)
...
def match_contour_in_area(image, scan_area, contour_signature):
    ...
    if best_match is not None:
        ...
        for i, point in enumerate(best_match):
            pt_coords = tuple(point[0])  
            cv2.circle(image, (pt_coords[0] + x_min, pt_coords[1] + y_min), 3, red_color, -1)
            cv2.putText(image, f'{i}', (pt_coords[0] + 4 + x_min, pt_coords[1] + y_min), cv2.FONT_HERSHEY_SIMPLEX, 0.3, red_color, 1)
