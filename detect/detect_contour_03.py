...
def match_contour_in_area(image, color_image, scan_area, contour_signature):
    x_min, y_min = scan_area[0]
    x_max, y_max = scan_area[1]
    region_of_interest = color_image[y_min:y_max, x_min:x_max]
    ...
    if best_match is not None:
        ...
        image[y_min:y_max, x_min:x_max] = cv2.cvtColor(region_of_interest, cv2.COLOR_BGR2GRAY)
...
def main():
    ...
    if image is not None:
        ...
        if selected_id in scan_areas:
            ...
            match_found, similarity = match_contour_in_area(image, color_image, scan_areas[selected_id], input_contour)
            if not match_found:
