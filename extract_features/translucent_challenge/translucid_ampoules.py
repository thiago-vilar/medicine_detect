...
class ExtractFeatures:
    ...
    def gaussian_filter(self, img_med):
        ...
        img_gaussian_gray = cv2.GaussianBlur(image_gray, (5, 5), 0)
        return img_gaussian_gray
    ...
    if processor.detect_stag():
        ...
        if homogenized is not None:
            ...
            if marked_image is not None:
                ...
                # gau_img = processor.gaussian_filter(cropped)
                # plt.imshow(gau_img, cmap='gray')
                # plt.title('Gaussian Filtered Image')
                # plt.show()

                # background_removed_2 = processor.remove_background(gau_img)
                # plt.imshow(cv2.cvtColor(background_removed_2, cv2.COLOR_BGR2RGB))
                # plt.title('Background Removed 2 -by Gaussian')
                # plt.show()

                sobel_img = processor.sobel_filter(cropped)
                plt.imshow(sobel_img, cmap='gray') 
                plt.title ('Sobel Image by Gaussian IMG')    
                plt.show()

                background_removed_3 = processor.remove_background(sobel_img)
                plt.imshow(cv2.cvtColor(background_removed_3, cv2.COLOR_BGR2RGB))
                plt.title('Background Removed 3 by Sobel')
                plt.show()
        
                otsu_img = processor.otsu(sobel_img)
                plt.imshow(otsu_img, cmap='gray')
                plt.title ('Otsu Image by Sobel')
                plt.show()

                background_removed_4 = processor.remove_background(otsu_img)
                plt.imshow(cv2.cvtColor(background_removed_4, cv2.COLOR_BGR2RGB))
                plt.title('Background Removed 4 - by Otsu')
                plt.show()

                # mask = processor.create_mask(background_removed_2)
