...
class ExtractFeatures:
    ...
    def medicine_measures(self, img, contours):
        ...
        for contour in contours:
            ...
            cv2.putText(img, f"{width_mm:.1f}mm x {height_mm:.1f}mm", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1)
        return img
            ...
            if marked_image is not None:
                ...
                if cropped is not None:
                    ...
                    if background_removed is not None:
                        ...
                        measured_image = processor.medicine_measures(contoured_image.copy(), contours)
                        plt.imshow(cv2.cvtColor(measured_image, cv2.COLOR_BGR2RGB))
                        plt.title('Measured Contours')
                        plt.show()
    else:
