...
class LoadMeasures:
    def __init__(self, measures_path):
        self.measures_path = measures_path

    def load_measures(self):
        with open(self.measures_path, 'rb') as f:
            measures = pickle.load(f)
        return measures


def compare_measures(extracted_measures, loaded_measures):
    ''' Compare extracted and loaded measures for both width and height, and print similarities. '''
    if not extracted_measures or not loaded_measures:
        print("No measures to compare.")
        return None

    # Comparar medidas
    similarities = []
    for i, (ext, load) in enumerate(zip(extracted_measures, loaded_measures)):
        width_similarity = 1 - (abs(ext[0] - load[0]) / max(ext[0], load[0]))  # Similaridade percentual em largura
        height_similarity = 1 - (abs(ext[1] - load[1]) / max(ext[1], load[1]))  # Similaridade percentual em altura
        similarities.append((i, width_similarity, height_similarity))
        
        print(f"Measure {i + 1}:")
        print(f"  Extracted: Width={ext[0]:.2f}mm, Height={ext[1]:.2f}mm")
        print(f"  Loaded:    Width={load[0]:.2f}mm, Height={load[1]:.2f}mm")
        print(f"  Similarity: Width={width_similarity:.2f}, Height={height_similarity:.2f}")
                ...
                if cropped is not None:
                    ...
                    if background_removed is not None:
                        ...
                        if contoured_image is not None and largest_contour is not None and largest_contour.size > 0:
                            ...
                            measures, measured_img = processor.medicine_measures(img_med, [largest_contour])
                            plt.imshow(cv2.cvtColor(measured_img, cv2.COLOR_BGR2RGB))
                            plt.title('Measured Medicine')
                            plt.show()

                            loader = LoadMeasures('features/medicine_measures/measured_medicine.pkl')
                            loaded_measures = loader.load_measures()
                            compare_measures(measures, loaded_measures)
