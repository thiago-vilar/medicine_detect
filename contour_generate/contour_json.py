...
def save_contours(contours):
    ...
    for i, contour in enumerate(contours):
        reshaped_contour = contour.reshape(-1, 2).tolist()  # Converting to list for JSON serialization

        # Save as JSON
        filename_json = os.path.join(directory, f"contour{i}.json")
        with open(filename_json, 'w') as f:
            json.dump(reshaped_contour, f)
        ...
        print(f"Contour {i} saved as JSON and PNG in {directory}/")
